    ...
    function onRequest(context) {
        if (context.request.method === 'GET') {
            ...
            try {
                ...
                pdfContent += '<tr>';
                pdfContent += '<td colspan="4">';
                pdfContent += 'The CLIENT shall be responsible for the safety and security of all the guests and their personal property. HCRSI shall NOT be responsible for any damage or loss of any merchandise, equipment, clothing, cell phones, gifts, other valuables left in the venue prior to during or after the function. HCRSI is not liable for any damages to the venue caused by his/her guests, and HCRSI is likewise free from any and all liabilities resulting to any loss or damage or injury that may be suffered by any guests, third persons or property prior to, during and after function.';
                pdfContent += '</td>';
                pdfContent += '</tr>';
                pdfContent += '<tr>';
                pdfContent += '<td colspan="4">';
                ...
                pdfContent += '<tr>';
                pdfContent += '<td colspan="4">';
                pdfContent += 'HCRSI further reserves the rights to substitute items that are unavailable in the open market or that exceed reasonable market costs. HCRSI will notify the CLIENT of any such changes if time allows.';
                pdfContent += '</td>';
                pdfContent += '</tr>';
                pdfContent += '<tr>';
                pdfContent += '<td colspan="4">';
                pdfContent += 'It shall be understood that everything is in order iif the HCRSI does not receive any written complaint from the CLIENT within 48 hours from the end of the function.';
                pdfContent += '</td>';
                pdfContent += '</tr>';
                pdfContent += '<tr>';
                pdfContent += '<td colspan="4">';
                pdfContent += 'The performance of this agreement by is subject to acts of God such as typhoons, earthquakes, fires, floods, or other disasters, and acts of man such as war, government regulations, strikes, civil disorders, curtailment of transportation facilities, or other emergencies making it inadvisable, illegal or impossible to uphold previous contractual agreements. HCRSI shall have no responsibility or liability for failure to supply any service when prevented from doing so by the occurrence of strikes, accidents, extra ordinary vehicular traffic or any cause beyond HCRSI’s control, or by orders of any governmental authority which may affect normal catering operations.';
                pdfContent += '</td>';
                pdfContent += '</tr>';
                pdfContent += '<tr>';
                pdfContent += '<td colspan="4">';
                pdfContent += 'HCRSI reserves the right to cancel a function if it has reason to believe that the exact nature of the event varies from the originally agreed upon, whether in writing or given verbally and/ or that the holding of such function may be against the law, public morals or public policy. In such case, the HCRSI shall have the right to forfeit any and all deposit or down payment already received as and by law of penalty and liquidated damages.';
                pdfContent += '</td>';
                pdfContent += '</tr>';
                pdfContent += '<tr>';
                pdfContent += '<td colspan="4">';
                ...
                pdfContent += '<td colspan="2"><strong>Hizon&apos;s Catering and Restaurant Services Inc.</strong></td>';
                pdfContent += '<td></td>';
                pdfContent += '<td></td>';
                pdfContent += '<td></td>';
                pdfContent += '<td></td>';
